---
layout: blog
title: "AWS 110: Amazon S3 Object Tags and Metadata"
date: 2025-11-21T11:50:32.609Z
---

## TLDR
S3 metadata and tags are key-value details you add to objects. Metadata includes AWS-provided information like `Content-Length` and `Content-Type`, as well as your custom fields that must start with `x-amz-meta`. Tags, also key-value pairs, are commonly used for access control and analytics. However, S3 does not support searching metadata or tags directly. To enable search functionality, you need to create an external index, such as using [Amazon DynamoDB](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Introduction.html). For more details, refer to the [Amazon S3 Documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingMetadata.html).


## Intro

In Amazon S3 you can store many objects. Each object can hold extra details that help you know more about it. These details come in two types. They are **metadata** and **object tags**.

## What Is Metadata? 

When you upload an object, you can add metadata. Metadata is a set of key-value pairs that provide information about the object. Some metadata is automatically provided by AWS, such as **Content-Length** and **Content-Type**, which indicate the file size and type. For more details on metadata, you can refer to the [Metadata Wikipedia page](https://en.wikipedia.org/wiki/Metadata).

You can also add your own metadata. If you do that each name must begin with **x amz meta**. This helps S3 know that you made it. For example you can add:

- `x-amz-meta-origin: paris`

This might help you remember where the file came from.

You can read this metadata when you download the object. It is a simple way to learn more about the file.

## What Are S3 Object Tags

Object tags are also key and value pairs (much like an Object in Javascript). They look like the tags you may have seen in other AWS services. Tags are used more often than metadata because they help with access rules and reports.

For example you can tag an object with:

- `Project: Blue`
- `PHI: True`

Tags can help you control who may read the object. They can also help with tools like S3 Analytics that group objects by tags.

## A Very Important Limit

Metadata and tags are useful. But S3 **cannot** search them. You cannot filter objects by metadata. You cannot filter objects by tags. S3 does not have that feature.

So what do you do if you want to search your bucket

You must build your own index in a database. DynamoDB is a common choice. You store all the metadata and tags in the database. When you search the database you find the objects you need. Then you fetch those objects from S3.

This design shows up in many exam questions and real systems.

## Conclusion

Metadata and tags can hold helpful facts about your S3 objects. They make it easier to manage and protect your data. Just remember that S3 itself cannot search them. For search you need an outside database like DynamoDB.
