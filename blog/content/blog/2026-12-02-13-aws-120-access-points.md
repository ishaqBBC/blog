---
layout: blog
title: "AWS 120: Amazon S3 Access Points"
date: 2026-02-13T15:05:44.175Z
---

## TLDR
S3 Access Points manage access to[S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/). Instead of building one complex [bucket policy](https://magicishaqblog.netlify.app/2025-28-03-aws-86-s3-security-bucket-policy/), you create separate access points for different teams or use cases. Each access point has its own policy and [DNS](https://magicishaqblog.netlify.app/2024-03-12-aws-58-DNS-name/) name. You can also restrict access to a private [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/) for added security.



## Introduction 

Picture a single [S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/) that holds finance data, sales data, and analytics reports. Different teams need access to different parts of that data. Finance needs its own files. Sales needs theirs. Analytics may need read only access to everything.

One option is to create a [bucket policy](https://magicishaqblog.netlify.app/2025-28-03-aws-86-s3-security-bucket-policy/). At first, this may work. But as more teams and more data are added, the policy grows. Over time, it becomes difficult to manage, review and update.

This is where S3 Access Points come in.

## S3 Access Points?

S3 Access Points allow you to create separate entry points into the same[ S3 bucket](https://magicishaqblog.netlify.app/2025-28-03-aws-86-s3-security-bucket-policy/). Each access point has its own permissions and network controls.

Instead of placing all security rules in a single [bucket policy](https://magicishaqblog.netlify.app/2025-28-03-aws-86-s3-security-bucket-policy/), you attach a policy to each access point.

Below is a diagram showing S3 Access points in action
![diagram of S3 Access points](/blog/src/images/120/120-1.png)

## A Practical Example

Let us return to our example.

We have one large [S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/)with:
...
We create a finance access point.
This access point has its own policy. The policy grants read and write access only to the finance prefix within the bucket.

Finance users connect through this access point and can only see their own data.

### Sales Access Point

Next, we create a sales access point.
It also has its own policy. This policy grants read and write access only to the sales prefix.

Sales users connect through their dedicated access point and cannot access finance data.

### Analytics Access Point

Now imagine an analytics team.
They need to read both finance and sales data, but they should not change anything.

We create an analytics access point with a read only policy. It allows access to both prefixes but does not permit write actions.

Each team now connects through its own access point, with permissions tailored to its needs.

## Why This Matters

By using access points, security management becomes more structured:

* Each access point has its own policy
* The S3 [bucket policy](https://magicishaqblog.netlify.app/2025-28-03-aws-86-s3-security-bucket-policy/) can remain simple
* Teams only see what they are meant to see

This approach makes it easier to scale. As your organisation grows, you can create new access points instead of expanding a single, complex policy.

## [DNS](https://magicishaqblog.netlify.app/2024-03-12-aws-58-DNS-name/) and Connectivity

Each access point has its own [DNS](https://magicishaqblog.netlify.app/2024-03-12-aws-58-DNS-name/) name. Applications connect using that [DNS](https://magicishaqblog.netlify.app/2024-03-12-aws-58-DNS-name/) name rather than the bucket name directly.

You can configure an access point in two main ways:

* Accessible over the internet
* Restricted to a Virtual Private Cloud, or [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/)

This flexibility allows you to control how traffic reaches your data.


## Private Access with [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/) Origin

For stricter security, you can create an access point that is only accessible from within a [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/).

For example, an EC2 instance inside a [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/) may need to access S3 without sending traffic over the public internet.

To make this work, you must create a [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/) endpoint. This endpoint allows private communication between your [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/) and Amazon S3.

The [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/) endpoint also has its own policy. That policy must allow access to the relevant[S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/) and access points.

In this setup, security exists at several levels:

* The S3 [bucket policy](https://magicishaqblog.netlify.app/2025-28-03-aws-86-s3-security-bucket-policy/)
* The access point policy
* The [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/) endpoint policy

Together, they provide strong and flexible control.

below is a diagram of the VPC.
![diagram of the VPC access point](/blog/src/images/120/120-2.png)

## Conclusion

S3 Access Points simplify security at scale.

Instead of complex [bucket policy](https://magicishaqblog.netlify.app/2025-28-03-aws-86-s3-security-bucket-policy/), you create multiple access points, each with a single purpose. Each access point has its own policy and [DNS](https://magicishaqblog.netlify.app/2024-03-12-aws-58-DNS-name/) name. You can also restrict access to private networks using [VPC](https://magicishaqblog.netlify.app/2023-15-11-aws-78-VPC/) origins and endpoints.
