---
layout: blog
title: "AWS 109: S3 Performance"
date: 2025-11-14T10:25:20.070Z
---



## TLDR  
 [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/) is very fast. It can handle many requests if you organise your files with different prefixes. You can speed uploads with multi part upload and transfer acceleration. You can speed downloads with byte range fetches.


## What Is S3 Baseline Performance?

 [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/) is a storage service on the cloud. It is built to scale quickly. This means it can handle a lot of traffic without slowing down.  
When you ask S3 for a file, you often get the first byte in about **100 to 200 milliseconds**. This is quite fast.

S3 also handles many requests every second. It lets you do:

- **3,500 PUT, COPY, POST, DELETE requests per second per prefix**
- **5,500 GET or HEAD requests per second per prefix**

This phrase *per second per prefix* is important. It sounds complex, so let us break it down.

 

## What Is a Prefix?

Think of a prefix as the path to your file inside your bucket.  
If your file sits in:
...
That prefix can handle 3,500 writes and 5,500 reads every second.
...
This is a **different** prefix. It also gets the same number of requests.

There is **no limit** to how many prefixes you can have.  
So if you spread your traffic across many prefixes, you can reach very high speeds.  
For example, with four prefixes, reads can reach **22,000 requests per second**.
...
### 1. Multi part upload  
Big files take time to upload. Multi part upload helps by breaking a big file into many small parts.  
These parts upload at the same time. When they reach S3, the service joins them back together.

Use multi part upload when your file is:

- Recommended for files over **100 MB**
- Required for files over **5 GB**

This method gives you faster and smoother uploads.

 

### S3 Transfer Acceleration  
If your file must travel a long way, such as from the United States to Australia, the trip can be slow. Transfer acceleration helps.

Here is how it works:

1. You send the file to the nearest **edge location**. These are spread across the world.  
2. The file travels the rest of the way inside the fast, private AWS network.  
3. Less public internet time means faster, safer travel.

Transfer acceleration also works with multi part uploads.

 

## Faster Downloads With Byte Range Fetches

Sometimes files are very large. You may not want the whole thing at once.  
Byte range fetches let you ask for only a part of the file.
...
Each chunk can be downloaded at the same time. This makes downloads faster.  
It also helps if one part fails. You only retry that small piece, not the whole file.

Another use is when you only need the first few bytes, such as a header that tells you what type of file it is.

 

## Final Thoughts

S3 is fast by default, and it can scale to huge numbers of requests.  
By using prefixes well, breaking files into parts, speeding long distance uploads, and fetching only the bytes you need, you can make S3 work even faster for you.
