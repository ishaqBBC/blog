---
layout: blog
title: "AWS 134: CloudFront VPC Origins for ALB, NLB, and EC2"
date: 2026-06-12T09:30:00.000Z
---

## TLDR

The modern and recommended way to connect [Amazon CloudFront](https://magicishaqblog.netlify.app/2026-03-06-aws-123-Amazon-CloudFront/) to application backends such as [ALB](https://magicishaqblog.netlify.app/ApplicationLoadBalancer/2023-08-18-aws-29-applicaton-load-balancer/), [NLB](https://magicishaqblog.netlify.app/NLB/2023-09-09-aws-32-network-load-balancer/), and [EC2](https://magicishaqblog.netlify.app/2023-02-24-aws-10-EC2/) is to use **VPC origins**. This keeps origins private inside your [VPC](https://magicishaqblog.netlify.app/2024-10-01-aws-81-vpc-round-up/) while still allowing global delivery through CloudFront edge locations. The older method used public origins restricted by CloudFront IP allow lists, which is more operationally heavy and carries higher exposure risk.

# Introduction

When we use [Amazon CloudFront](https://magicishaqblog.netlify.app/2026-03-06-aws-123-Amazon-CloudFront/), one common question is:

How do we connect CloudFront to an [Application Load Balancer](https://magicishaqblog.netlify.app/ApplicationLoadBalancer/2023-08-18-aws-29-applicaton-load-balancer/), [Network Load Balancer](https://magicishaqblog.netlify.app/NLB/2023-09-09-aws-32-network-load-balancer/), or an [EC2](https://magicishaqblog.netlify.app/2023-02-24-aws-10-EC2/) instance as an origin?

Today, there are two approaches:

1. A newer, recommended model using **VPC origins**
2. A legacy model using **public origins** with restricted access

In practice, VPC origins are generally the better design from both a security and operations perspective.

## The Modern Approach: VPC Origins

VPC origins allow CloudFront to deliver content from applications hosted in **private subnets** inside a [VPC](https://magicishaqblog.netlify.app/2024-10-01-aws-81-vpc-round-up/).

This means your backend systems can remain private and do not need direct public internet exposure.

With this model, CloudFront can route traffic to:

- Private [Application Load Balancers (ALB)](https://magicishaqblog.netlify.app/ApplicationLoadBalancer/2023-08-18-aws-29-applicaton-load-balancer/)
- Private [Network Load Balancers (NLB)](https://magicishaqblog.netlify.app/NLB/2023-09-09-aws-32-network-load-balancer/)
- Private [EC2](https://magicishaqblog.netlify.app/2023-02-24-aws-10-EC2/) instances

## How the Traffic Flow Works

At a high level:

1. Users access your CloudFront distribution.
2. Requests arrive at the nearest CloudFront edge location.
3. CloudFront forwards requests to a configured VPC origin.
4. The VPC origin routes traffic to your private backend in VPC subnets.

Your application stays internal, while CloudFront acts as the internet-facing entry point.

This model gives you tighter control over what is exposed publicly and creates a stronger network boundary around your workloads.

## Why VPC Origins Are Stronger

From a security architecture point of view, VPC origins offer several practical advantages:

- **Reduced public surface area**: backends remain private.
- **Cleaner perimeter design**: CloudFront is the public layer, origin infrastructure remains internal.
- **Lower operational overhead**: less manual IP allow-list maintenance.
- **Lower misconfiguration risk**: fewer chances of unintentionally overexposing origin resources.

## The Previous Method: Public Origins + CloudFront IP Restriction

Before VPC origins were available, the common method was to make the origin publicly reachable and then restrict access to CloudFront source IP ranges.

### Public EC2 as Origin

For an EC2 origin, the instance had to be public.

The [security group](https://magicishaqblog.netlify.app/2023-03-10-aws-12-security-groups) would then be configured to allow inbound traffic only from CloudFront edge location public IP ranges. Teams needed to regularly reference and maintain AWS-published CloudFront IP ranges.

### Public ALB as Origin

For an ALB origin, the ALB itself was public, while target EC2 instances could still be private.

[Security groups](https://magicishaqblog.netlify.app/2023-03-10-aws-12-security-groups) were set up so:

- The ALB allowed CloudFront public IP ranges.
- Private backend instances accepted traffic from the ALB.

This architecture worked, but introduced additional complexity and maintenance burden.

## Limitations of the Legacy Model

The older public-origin approach had clear drawbacks:

- **Continuous maintenance** of CloudFront IP allow lists.
- **Higher configuration complexity** across [security groups](https://magicishaqblog.netlify.app/2023-03-10-aws-12-security-groups).
- **Greater accidental exposure risk** if security groups were broadened incorrectly.

In short: it was functional, but operationally fragile compared to VPC origins.

## VPC Origins vs Public Origins

| Area                                 | VPC Origins (Recommended) | Public Origins (Legacy) |
| ------------------------------------ | ------------------------- | ----------------------- |
| Origin exposure                      | Private                   | Public                  |
| Security group complexity            | Lower                     | Higher                  |
| CloudFront IP allow-list maintenance | Not central to the model  | Required                |
| Misconfiguration blast radius        | Lower                     | Higher                  |
| Preferred for new designs            | Yes                       | No                      |

## Conclusion

If you are designing a new CloudFront architecture for ALB, NLB, or EC2, use **VPC origins** as the default choice.

The legacy public-origin model is still useful to understand, especially for existing environments, but the VPC origin approach provides a cleaner and more secure baseline for modern deployments.

For deeper implementation details, use the official AWS CloudFront documentation and VPC origin guidance when building production configurations.
