---
layout: blog
title: "AWS 99: CLI Profiles"
date: 2025-07-25T09:23:47.917Z
---


## TLDR

If you're juggling multiple AWS accounts, don't overwrite your default credentials. Instead, use **named profiles** to configure and manage each set of credentials separately. Then simply add `--profile <profile-name>` to your [AWS CLI](https://magicishaqblog.netlify.app/2023-10-03-aws-7-cli/) commands to switch accounts.
...
By default, the AWS CLI stores credentials in:

- `~/.aws/credentials`
- `~/.aws/config`

Here’s what a default setup looks like:
...
AWS profiles let you store multiple sets of credentials under different names, allowing you to switch between them easily.

####
 Adding a Second Profile

1. **Run AWS Configure with a Profile Flag**
...
* `Default output format` (e.g. `json`, or just press Enter)

3. **Resulting Configuration Files**

Your `~/.aws/credentials` will now look like this:
...
### Using the Profile in CLI Commands

By default, CLI commands use the `[default]` profile:

```bash
aws s3 ls
```

To use another profile, append the `--profile` flag:

```bash
aws s3 ls --profile my-other-account
```

This will execute the command using the credentials and configuration tied to `my-other-account`.


### Why It Matters

While this approach won’t appear on AWS certification exams, it’s essential for day-to-day development. Whether you're managing production vs. test environments, or juggling freelance clients, using profiles makes switching accounts safe and efficient.

### Closing notes

You can define a many profiles as you need. Just remember to name them clearly and use the `--profile` flag consistently in your commands.
