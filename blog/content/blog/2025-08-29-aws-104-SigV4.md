...
layout: blog
title: "AWS 104: SigV4"
date: 2025-08-29T10:11:53.198Z
---


## TLDR
Making an API request to AWS, you wil need to be **signed** (most of the time) so that AWS knows who you are and can verify you’re authorised. The signing process, known as **Signature Version 4 (SigV4)**, uses your AWS credentials (access key + secret key). The signed request can be sent in two ways:  
1. In the **Authorization header** of the HTTP request.  
2. In the **query string** of the request URL (via `X-Amz-Signature`).  

This signing is handled automatically if you use the AWS CLI or SDKs. You need to understand it if you’re creating requests manually or troubleshooting.



## How it works

 An **AWS HTTP API**—which underpins all AWS services—you must convince AWS of two things:  

1. **Who you are**  
2. **That you’re allowed to make the request**  

To achieve this, the request is **signed**. Signing involves taking your **AWS access key** and **secret key** and generating a cryptographic signature that travels with the request.  

AWS checks this signature, and if it matches expectations, your request proceeds.  

## When do you need to sign?

- **Public resources**: Some calls don’t need signing , for example; downloading a publicly available file from Amazon S3.  
- **Most cases**: The vast majority of AWS API calls do require signing.  

If you’ve been using the **AWS CLI** or any official **SDK**, you’ve never had to worry about this. They handle signing behind the scenes. But when making raw HTTP requests yourself, signing becomes essential.


## SigV4: The signature system

AWS uses a system called **Signature Version 4 (SigV4)**.  
The details are mathematically complex, but the key point is simple:  

- SigV4 ensures **integrity** (the request wasn’t tampered with)  
- SigV4 ensures **authenticity** (the request really came from you)  


## Two ways to send your signature

Once the SigV4 signature has been generated, it can be sent to AWS in two ways:  

### 1. **HTTP Authorization Header**
The signature is placed inside the request header. This is what the **AWS CLI** and **SDKs** do automatically.  

Example (simplified):  
```

Authorization: AWS4-HMAC-SHA256 Credential=YOUR\_KEY\_ID/DATE/REGION/SERVICE/aws4\_request, SignedHeaders=..., Signature=...

```

### 2. **Query String Parameters**
The signature can also be embedded directly into the URL as query parameters. This is common when generating **pre-signed URLs**, which let others access your resources without needing AWS credentials.  

Example (simplified):  
```

[https://bucket.s3.amazonaws.com/coffee.jpg?X-Amz-Algorithm=AWS4-HMAC-SHA256\&X-Amz-Credential=...\&X-Amz-Date=...\&X-Amz-Signature=](https://bucket.s3.amazonaws.com/coffee.jpg?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=...&X-Amz-Date=...&X-Amz-Signature=)...

```

This method is used for **temporary access to S3 objects**, such as sharing a file link that automatically expires.


## Real-world example: Pre-signed S3 URL

in Amazon S3: `coffee.jpg` is stored.  
When you open it via a pre-signed URL in your browser, you’ll notice:  

- `X-Amz-Algorithm` --> confirms it’s using SigV4  
- `X-Amz-Date` --> shows when the request was signed  
- `X-Amz-Expires` --> determines when the link will stop working  
- `X-Amz-Credential` --> ties the request to your AWS account  
- `X-Amz-Signature` --> the actual signature  

Together, these fields prove to AWS that your request is valid.


## Conclusion


- Your request needs to be stamped with a **signature**.  
- That signature proves your **identity** and **permissions**.  
- It can travel via the **Authorization header** or in a **query string** (`X-Amz-Signature`).  

For most developers, the **AWS CLI and SDKs** take care of all this automatically.
