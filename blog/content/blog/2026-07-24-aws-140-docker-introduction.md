---
layout: blog
title: "AWS 140: Docker Introduction"
date: 2026-07-24T04:36:26.872Z
---

## TLDR

Docker is a way of making containers for applications for consistent deployment across environments. AWS provides these services:

- **[Amazon ECS](https://docs.aws.amazon.com/ecs/)** – AWS's native container orchestration service
- **[Amazon EKS](https://docs.aws.amazon.com/eks/)** – Managed Kubernetes service
- **[AWS Fargate](https://docs.aws.amazon.com/fargate/)** – Serverless container runtime
- **[Amazon ECR](https://docs.aws.amazon.com/ecr/)** – Managed container registry

Containers offer lightweight isolation by sharing the host OS, a different approach to a [virtual machine](https://en.wikipedia.org/wiki/Virtual_machine)
...
[Docker](https://docs.docker.com/) is a software platform that packages applications into **containers**. A container includes the application itself, along with everything it needs to run, such as libraries, dependencies, configuration files, and runtime components.

The primary advantage is consistency. Once an application has been containerised, it behaves the same regardless of where it is deployed. Whether it runs on a developer's workstation, a testing environment, or a cloud server, the execution environment remains largely unchanged.

This approach reduces compatibility issues, simplifies deployment, and makes applications easier to maintain throughout their lifecycle.

![diagram one of docker](/blog/src/images/140/140-1.png)

## Why Developers Use Containers

Containers have become a standard building block in modern software development for several reasons.

They support **microservices architectures**, where applications are divided into smaller, independent services. Each service can be packaged into its own container, allowing teams to develop, deploy, and update components without affecting the rest of the system.

Containers are also valuable when organisations migrate existing applications from on-premises infrastructure to the cloud. Instead of rebuilding software for a new environment, teams can package applications into containers and move them with minimal changes.

Beyond these scenarios, containers provide a straightforward way to deploy applications consistently across multiple environments.

## How Docker Works

A [Docker](https://docs.docker.com/) environment begins with a server running the [Docker](https://docs.docker.com/) Engine (sometimes called the [Docker](https://docs.docker.com/) daemon). This server could be an [Amazon EC2](https://magicishaqblog.netlify.app/2023-02-24-aws-10-EC2/) instance, a physical machine, or another supported operating system.

Once [Docker](https://docs.docker.com/) is installed, multiple containers can run on the same host. Each container operates independently while sharing the host operating system.

For example, a single server might run:

- A Java application
- A Node.js application
- A MySQL database
- Multiple copies of the same application to handle increased demand

Although each container appears isolated, they all use the same underlying operating system. This shared architecture makes containers significantly lighter than traditional virtual machines.

## Docker Images and Repositories

Before an application can run inside a container, it must first be packaged into a **[Docker](https://docs.docker.com/) image**.

A [Docker](https://docs.docker.com/) image is a read-only template containing everything required to start a container. Once created, the image is stored in a container registry so it can be downloaded whenever needed.

Two of the most common repositories are:

- **Docker Hub** – the most widely used public registry, offering thousands of pre-built images for operating systems, programming languages, databases, and development tools.
- **Amazon Elastic Container Registry (Amazon ECR)** – AWS's managed container registry. It supports both private repositories and a public image gallery for sharing container images.

Using a central repository allows development teams to distribute the exact same application image across different environments without rebuilding it.

## Docker vs Virtual Machines

Containers and virtual machines both provide application isolation, but they achieve it differently.

A virtual machine includes an entire guest operating system running on top of a hypervisor. Each VM has dedicated system resources, making it highly isolated but relatively heavy.

A [Docker](https://docs.docker.com/) container, by contrast, shares the host operating system with other containers. Rather than running a complete operating system for every application, [Docker](https://docs.docker.com/) isolates processes while relying on the host kernel.

The differences have practical consequences.
...
- Allow many more workloads to run on the same server

This efficiency explains why containers have become the preferred choice for many cloud-native applications. Although virtual machines generally provide stronger isolation, containers offer greater density and faster deployment.

![diagram one of docker 2](/blog/src/images/140/140-2.png)

## Building and Running Docker Containers

Working with [Docker](https://docs.docker.com/) follows a straightforward process.

First, a developer creates a **Dockerfile**, which describes how the container should be built. The file specifies the base image, application files, dependencies, and configuration.

[Docker](https://docs.docker.com/) then builds the Dockerfile into a [Docker](https://docs.docker.com/) image.

The image is uploaded to a container registry such as Docker Hub or Amazon ECR, where it can be stored and versioned.

Whenever the application needs to be deployed, the image is downloaded from the registry and started as a running container.

The process can be summarised as follows:

1. Create a Dockerfile.
2. Build the [Docker](https://docs.docker.com/) image.
3. Push the image to a registry.
4. Pull the image when required.
5. Run the image as a container.

This workflow allows every deployment to use the same packaged application, reducing inconsistencies between environments.

![diagram one of docker 3](/blog/src/images/140/140-3.png)

## Managing Containers on AWS

Running a handful of containers manually is manageable. Operating hundreds or thousands across multiple servers is not.

AWS provides several services that automate container deployment and management.

### Amazon ECS

Amazon Elastic Container Service (ECS) is AWS's native container orchestration platform. It manages container scheduling, scaling, networking, and availability without requiring organisations to manage complex orchestration software themselves.

For teams working primarily within AWS, ECS offers a tightly integrated experience with other AWS services.

![ec2 icon](/blog/src/images/140/140-6.png)

### Amazon EKS

Amazon Elastic Kubernetes Service (EKS) is AWS's managed implementation of Kubernetes.

Kubernetes has become the industry's leading container orchestration platform, particularly for organisations that need portability across cloud providers or already use Kubernetes in existing environments.

EKS removes much of the operational burden involved in running Kubernetes while retaining its flexibility and extensive ecosystem.
![eks icon](/blog/src/images/140/140-4.png)

### AWS Fargate

AWS Fargate provides a serverless way to run containers.

Rather than provisioning and maintaining EC2 instances, developers simply define the resources required by their containers. AWS handles the underlying infrastructure automatically.

Fargate works with both Amazon ECS and Amazon EKS, making it an attractive option for organisations that want to minimise operational overhead.

![fargate icon](/blog/src/images/140/140-5.png)

### Amazon ECR

Amazon Elastic Container Registry (ECR) serves as the central repository for Docker images within AWS.

It integrates closely with ECS and EKS, simplifying authentication, image storage, and deployment pipelines.

## Conclusion

Docker has transformed how software is packaged and deployed. By encapsulating applications and their dependencies into portable containers, it removes many of the compatibility challenges that once complicated software deployment.

Within AWS, services such as Amazon ECS, Amazon EKS, AWS Fargate, and Amazon ECR build upon Docker's foundation, providing scalable tools for deploying and managing containerised workloads in production.

Understanding how these services relate to one another is an important step towards mastering modern cloud infrastructure. Docker provides the packaging format; AWS supplies the platform for running those containers reliably at scale.
