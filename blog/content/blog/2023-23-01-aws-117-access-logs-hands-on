---
layout: blog
title: "AWS 177: Access Loqs: Hands On"
date: 2026-01-23T14:56:27.739Z
---


## TLDR

* Create a separate [S3 Bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/) to store [access logs](https://magicishaqblog.netlify.app/2026-01-16-aws-116-amazon-s3-access-LOGS/)
* Turn on *Server access logging* for an existing bucket
* Choose the logging bucket as the destination
* Activity in your bucket is recorded, but logs take time to appear
* Log files show who accessed the bucket, when, and what they did

## Introduction

Based on our previous [blog post](https://magicishaqblog.netlify.app/2026-01-16-aws-116-amazon-s3-access-LOGS/) about Amazon S3 Access Logs. [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/) [access logs](https://magicishaqblog.netlify.app/2026-01-16-aws-116-amazon-s3-access-LOGS/) help you understand what is happening inside your [storage buckets](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/). They record every request made to a bucket, such as uploading a file or opening an object. In this short guide, we’ll walk through how to set them up and what to expect afterwards.

### Create a Logging Bucket

Create an [S3 Bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/). This bucket will store the [access logs](https://magicishaqblog.netlify.app/2026-01-16-aws-116-amazon-s3-access-LOGS/).

 Like a notebook. Instead of keeping notes inside the main bucket, [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/) writes them into a separate one. 

*Once the bucket is created, leave it open in one browser tab.*

![image of a bucket being created for an access log](/blog/src/images/117/117-1.png)

### Enable Access Logging on Another Bucket

In a second tab, open one of your existing [S3 Buckets](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/).

* Go to **Properties**
* Scroll down to **Server access logging**
* Click **Edit** and enable logging

[Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/) will let you know that it needs to update the bucket policy. This is important, as it allows S3 to write log files to your logging bucket.

![image of a enabling the logging on a secondary bucket](/blog/src/images/117/117-2.png)


### Choose Where the Logs Go

Now you must tell [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/) where to send the logs.

* Select the logging bucket you created earlier
* Confirm the region (for example, *eu-west-1*)
* Leave the prefix empty unless you want logs stored in a folder
* Keep the default log file format

For most users, the default settings work just fine. Click **Save changes** when you’re done.

Your bucket is now being logged.
...
Every action creates activity, and that activity is logged.

If you refresh the logging bucket straight away, you may see nothing. That’s normal.

### Check Permissions

**Access logs are not delivered instantly. They are sent in batches, which can take minutes or even hours.**

While waiting, you can check that everything is set up correctly:

* Open the logging bucket
* Go to **Permissions**
* Look at the **Bucket policy**

You should see a policy that allows the [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/) logging service to write objects to the bucket. This confirms that logging is working as expected.
...
After some time, refresh the logging bucket again.

You should now see many new files. These are your [access logs](https://magicishaqblog.netlify.app/2026-01-16-aws-116-amazon-s3-access-LOGS/). Open one, and you’ll see detailed information such as:
...
While the string will be hard to read. The information is there.

![image of logs](/blog/src/images/117/117-3.png)


### Conclusion

S3 [access logs](https://magicishaqblog.netlify.app/2026-01-16-aws-116-amazon-s3-access-LOGS/) are a powerful tool, but they require patience. Once enabled, they quietly record activity in the background and deliver detailed records later.

That’s all for S3 [access logs](https://magicishaqblog.netlify.app/2026-01-16-aws-116-amazon-s3-access-LOGS/). 
