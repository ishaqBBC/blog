...
## TLDR
Amazon S3’s [*MFA Delete*](https://magicishaqblog.netlify.app/2025-aws-114-MFA-Delete/) adds an extra layer of protection so that even if someone gets access to your account, they still cannot permanently delete data without a one-time [MFA](https://magicishaqblog.netlify.app/2025-08-01-aws-100-CLI-MFA-IAM/) code. You cannot enable MFA Delete in the AWS web console. You must use the [AWS CLI](https://magicishaqblog.netlify.app/2025-08-01-aws-100-CLI-MFA-IAM/), log in as the **root user**, and have MFA already set up. It’s powerful, a bit awkward to configure, and should be used carefully.



## Introduction
Based on our previous blog post : [*MFA Delete*](https://magicishaqblog.netlify.app/2025-aws-114-MFA-Delete/).
In cloud security, a single wrong click can delete data forever. Amazon Web Services (AWS) offers a feature called [*MFA Delete*](https://magicishaqblog.netlify.app/2025-aws-114-MFA-Delete/) to reduce that risk, but it comes with strict rules.

We will go through MFA delete



## What Is MFA Delete?
MFA stands for **Multi-Factor Authentication**. It usually means entering a one-time code from an app on your phone, in addition to your password.
When [*MFA Delete*](https://magicishaqblog.netlify.app/2025-aws-114-MFA-Delete/) is enabled on an S3 bucket:

* You **cannot permanently delete objects** without an MFA code
* You **cannot turn off versioning** without an MFA code
* Normal uploads and deletes still work, but permanent deletion is blocked

This feature is designed to protect your data from:
...
MFA Delete only works if **bucket versioning is enabled**.

1. Create a new S3 bucket [STEPS HERE](https://magicishaqblog.netlify.app/2025-03-21-aws-85-Amazon-s3-hands-on/)
2. Choose a unique name (for example: `demo-magicishaq-mfa-delete-2020`)
3. Select a region (such as `eu-west-1`)
4. Enable **bucket versioning** (below is a screenshot)
![image if enabling bucket version in amazon s3](/blog/src/images/115/115-1.png)
5. Create the bucket

At this point, versioning is on, but MFA Delete is still disabled.





## 2: Why You Can’t Use the AWS Console

If you open the bucket settings in the AWS web console, you will see this:

* Versioning: **Enabled**
* MFA Delete: **Disabled**
* Edit option:  Not available

This is expected. **AWS does not allow enabling MFA Delete from the console**. You must use the **AWS CLI**.



## 3: Prepare Your Root Account (Important)

This is the most sensitive part.

To enable MFA Delete, you must:

* Log in as the **root user** [STEPS HERE](https://magicishaqblog.netlify.app/2023-01-23-aws-2-getting-started/)
* Have **MFA already configured** on the root account [STEPS HERE](https://magicishaqblog.netlify.app/2025-08-01-aws-100-CLI-MFA-IAM/)

* Temporarily create **root access keys**
![screen shot of creating access keys](/blog/src/images/115/115-2.png)

> AWS strongly advises against using root access keys.
> Only do this for MFA Delete, then delete the keys immediately.

From the IAM security credentials page:

* Confirm your MFA device is active
* Copy the **ARN** of the MFA device
...
Using the temporary root access keys: Inside the [AWS CLI](https://magicishaqblog.netlify.app/2023-10-03-aws-7-cli/) 

1. Run `aws configure --profile root-mfa-delete-demo`
2. Enter:

   * Access key ID
   * Secret access key
   * Default region (for example: `eu-west-1`)
...
## 5: Enable MFA Delete Using the CLI

Now comes the key command.

You run an S3 versioning configuration command that:
...
* Requires:
  ...
  * The MFA device ARN
  * A current MFA code from your app

If the MFA code is expired, the command will fail. Just wait for the next code and try again.

When it succeeds, MFA Delete is active.
Below is an example
```bash
aws s3api put-bucket-versioning --bucket demo-magicishaq-mfa-delete-2020 --versioning-configuration Status=Enabled,MFADelete=Enabled \
--mfa "arn:aws:iam::001736599714:mfa/root-account-mfa-device 710343" --profile root-mfa-delete-demo
```
`arn:aws:iam::001736599714:mfa/root-account-mfa-device` is taken from the security credentials in [IAM](https://docs.aws.amazon.com/IAM/latest/UserGuide/introduction.html)

`710343` is a placeholder number - yours will be different and available on your authentication app

Below is a screen shot of what you are looking for

![Screenshot of security credentials in IAM](/blog/src/images/115/115-3.png)




## 6: Confirm MFA Delete Is Enabled

Go back to the S3 console and refresh the bucket settings.
...
![Screenshot of bucket versioning with MFA delete enabled](/blog/src/images/115/115-4.png)



## 7: Test What MFA Delete Actually Does
...
* But only a **delete marker** is added

This is normal behavior with versioning.

Next, try deleting a **specific version** of the file (a permanent delete).

AWS will block you.

You’ll see a message saying the action is denied because **MFA Delete is enabled**.

This is exactly what we want.



## 8: Disabling MFA Delete (Also Requires MFA)

To permanently delete versions again, you must disable MFA Delete.
...
* Permanent deletes work again
* The console will show MFA Delete as disabled


```bash
aws s3api put-bucket-versioning --bucket demo-magicishaq-mfa-delete-2020 --versioning-configuration Status=Enabled,MFADelete=Disabled \
--mfa "arn:aws:iam::001736599714:mfa/root-account-mfa-device 710343" --profile root-mfa-delete-demo
```
`arn:aws:iam::001736599714:mfa/root-account-mfa-device` is taken from the security credentials in [IAM](https://docs.aws.amazon.com/IAM/latest/UserGuide/introduction.html)
...
## Final and Most Important Step: Clean Up

Before you finish:

* **Deactivate and delete the root access keys**
* Never leave root keys active
* Never share them


## Conclusion
MFA Delete is a security feature for S3 buckets, designed to prioritize data protection over ease of use.

Use MFA Delete when:

* Accidental deletion would be costly
* You want strong safeguards, even from yourself
