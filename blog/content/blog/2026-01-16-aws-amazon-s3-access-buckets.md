...
## TLDR
Amazon S3 Access Logs record every request made to an [S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/), whether allowed or denied. Logs are stored in a separate [S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/) in the same AWS region and can be analysed with tools like [Amazon Athena](https://docs.aws.amazon.com/athena/latest/ug/what-is.html). Never log to the same bucket you are monitoring, as this creates an infinite logging loop and high costs.

---

### What are S3 Access Logs?

Amazon S3 Access Logs are used for auditing and monitoring. They provide a detailed record of all requests made to an [S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/). This includes requests from any AWS account and covers both successful and failed attempts.

Each request is written as a log file and saved to a separate [S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/) called the *logging bucket*. These logs help answer:
...
Access logs are used for security reviews, compliance checks, and troubleshooting. Once stored, the log files can be analysed using AWS data tools such as [Amazon Athena](https://docs.aws.amazon.com/athena/latest/ug/what-is.html). This allows teams to run queries and look for patterns without moving the data elsewhere.

### How does logging work?

To use S3 Access Logs, logging must be enabled on the source bucket. After that:

- Requests are made to the [S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/).
- Each request is recorded by Amazon S3.
- Log files are delivered to the target logging bucket.

Both the source bucket and the logging bucket must be in the same [AWS region](https://magicishaqblog.netlify.app/2023-01-23-aws-2-getting-started/#regions). below is a diagram:
![diagram of s3 access logs](/blog/src/images/116/116-1.png)


### Log File Format Example

The log files follow a fixed format defined by AWS. This format includes details such as requester, bucket name, request time, and response status. Below is an example of a log entry:

```bash
79a5EXAMPLEbucket [10/Oct/2023:13:55:36 +0000] 192.0.2.3 arn:aws:iam::123456789012:user/ExampleUser REST.GET.OBJECT example-object.txt "GET /example-object.txt HTTP/1.1" 200 - 12345 6789 9 "-" "S3Console/0.4" -
```

**Explanation of fields:**
- **Bucket Name:** `79a5EXAMPLEbucket` - The name of the bucket being accessed.
- **Timestamp:** `[10/Oct/2023:13:55:36 +0000]` - The date and time of the request.
- **Requester:** `192.0.2.3` - The IP address of the requester.
- **Requester ARN:** `arn:aws:iam::123456789012:user/ExampleUser` - The AWS Identity and Access Management (IAM) user making the request.
- **Operation:** `REST.GET.OBJECT` - The type of operation performed.
- **Object Key:** `example-object.txt` - The object being accessed.
- **HTTP Request:** `"GET /example-object.txt HTTP/1.1"` - The HTTP request details.
- **HTTP Status Code:** `200` - The response status code (e.g., 200 for success).
- **Bytes Sent:** `12345` - The size of the response in bytes.
- **Bytes Received:** `6789` - The size of the request in bytes.
- **Total Time:** `9` - The total time taken to process the request (in milliseconds).
- **User Agent:** `"S3Console/0.4"` - The user agent making the request.


### Reminder

The logging bucket must **never** be the same as the bucket being monitored.

If the same bucket is used for both purposes, every log file written will itself generate a new access log. This creates an infinite logging loop. As a result, the bucket will grow , and this mean more costs. Below is a diagram to illustrate.

![example of the loop causing high costs in access buckets](/blog/src/images/116/116-2.png)

*For this reason, always use a separate bucket dedicated to logging.*

### Conclusion

S3 Access Logs are used for tracking activity on [S3 bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/)s. When configured , they provide clear insight into bucket usage and support auditing and analysis. Careful setup, especially using a separate logging bucket, is essential to avoid unnecessary costs.
