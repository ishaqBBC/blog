...
[Multi-Factor Authentication](https://support.microsoft.com/en-gb/topic/what-is-multifactor-authentication-e5e39437-121c-be60-d123-eda06bddf661) (MFA) adds an extra layer of security. While MFA is commonly used in the AWS Console, it can also be enforced for CLI/API usage. This requires generating **temporary credentials**.

## How to

First, assign an MFA device to your IAM user:

1. Go to **IAM > Users** and select your user.
...
These credentials are temporary (default 1 hour).


Create a new CLI profile with these credentials:
...
Then, manually add the session token to your credentials file:
...
Use your MFA-authenticated profile like this:

```bash
aws s3 ls --profile mfa
```

This will use the temporary credentials for the duration of their validity.
...
This API call allows MFA-based temporary access. Once configured, you can safely interact with AWS services via the CLI using short-lived credentials.
