---
layout: blog
title: "AWS 97: EC2 Instance Metadata"
date: 2025-07-11T09:23:19.968Z
---

## TLDR
EC2 Instance Metadata [(IMDS)](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-metadata.html) is a way for your cloud computer (an EC2 instance) to learn about itself. It doesn’t need special permissions. It just asks a secret web address for details. There are two versions: IMDSv1 (easy but less secure) and IMDSv2 (more steps but safer).


## Refreshers EC2s?

**EC2** think of it as a computer living in the cloud of Amazon Web Services (AWS). To run websites, apps ..etc just like a normal computer.

EC2 is clever because:
It can tell you things about itself, without you needing to log in or give it a password. That’s where **IMDS** comes in.

Read more about EC2 Instances [HERE](https://magicishaqblog.netlify.app/2023-02-24-aws-10-EC2/)


## The Instance Metadata Service

**IMDS** stands for **Instance Metadata Service**. It’s a helper that lives inside every [EC2](https://magicishaqblog.netlify.app/2023-02-24-aws-10-EC2/) computer. It knows:
...
Your EC2 instance communicates with a special web address:
...
* **User data** = A script or instructions that run when the instance starts

They both live behind the same special address, but we’re focusing on metadata for now.
...
* Simple and easy
* Just go to the URL and done!
* But not very secure

###  IMDSv2

* More secure
* You must *ask for permission* first (get a token)
* Then you can use that token to ask for metadata

From 2023, **IMDSv2 is the default** on new EC2 instances.

It’s a bit more work, but it keeps your cloud computer safer and secure.

More Information about IMDS version 1 and 2 [HERE](https://aws.amazon.com/blogs/security/get-the-full-benefits-of-imdsv2-and-disable-imdsv1-across-your-aws-infrastructure/)


### Conclusion

Working with EC2 and using metadata is powerful. Build smarter apps, automatic scripts, and make things more secure all by asking your instance to tell you about itself.

Kit comes standard with all EC2 instances.

In the next post, I will show you *how* to use IMDS step by step. You’ll see how to fetch info using both IMDSv1 and IMDSv2.
