...
Today we look at how Amazon S3 protects/encrypts your files. 
Think of S3 as an online cupboard where people keep photos, videos and documents. But like any cupboard, it needs to stay safe and stop intruders. This is done through encryption. [This is when data is changed into secret code so no one else can read it unless you have the key](https://en.wikipedia.org/wiki/Encryption).

## Four Ways to Encrypt an Object using Amazon s3

Amazon S3 has four main ways to encrypt your files.

### 1. Server Side Encryption with S3 Keys  
This is called [**SSE-S3**](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingServerSideEncryption.html). Here, Amazon manages the encryption keys for you. It is enabled by default for new buckets and objects. To use it, you simply include the `x-amz-server-side-encryption` header with the value `AES256` in your request. For more details, refer to the [AWS documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingServerSideEncryption.html).
![diagram of SSE-S3](/blog/src/images/111/111-1.png)

 
### 2. Server Side Encryption with KMS Keys  
This is called [**SSE-KMS**](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingKMSEncryption.html). Here, you use AWS Key Management Service (KMS) to manage your encryption keys. This gives you more control because you can define who can use the keys and monitor their usage through AWS CloudTrail logs. However, keep in mind that each time S3 uses the key, it counts as a KMS request. If you perform a high volume of read or write operations, you may need to consider the KMS request limits.
![diagram of SSE-KMS](/blog/src/images/111/111-2.png)
 
### 3. Server Side Encryption with Your Own Key  
This is called [**SSE-C**](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html). You create and manage the encryption key outside of Amazon. When you upload or download a file, you provide the key to S3. Amazon does not store your key, so you are responsible for its safekeeping. To ensure the key remains secure during transmission, you must use HTTPS. For more details, refer to the [AWS documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html).
![diagram of SSE-C](/blog/src/images/111/111-3.png)
 
### 4. Client Side Encryption  
This is called [**Client Side Encryption**](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingClientSideEncryption.html). You encrypt the file on your own computer before sending it to S3, and you also decrypt it yourself. Amazon never sees the encryption key. To make this process easier, you can use libraries such as the [Amazon S3 Encryption Client](https://docs.aws.amazon.com/sdk-for-java/v1/developer-guide/s3-encryption-client.html) or other SDKs that support client-side encryption. For more details, refer to the [AWS documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingClientSideEncryption.html).
![diagram of Client side encryption](/blog/src/images/111/111-4.png)

## Encryption While Travelling

Your data also needs to be safe while it moves between you and S3. This is called using HTTPS. It creates a secure path so no one else can read your data on the way. Also called [**SSL/TLS**](https://docs.aws.amazon.com/AmazonS3/latest/userguide/serv-side-encryption.html#encryption-in-transit).

Most tools already use HTTPS. You can also tell your bucket to block any request that is not secure. You do this with a bucket policy that only allows secure transport.

Here is an example bucket policy to enforce secure transport:
...
Replace `your-bucket-name` with the name of your S3 bucket. This policy denies any request that does not use HTTPS (`aws:SecureTransport` set to `false`).

## Conclusion

Encryption keeps your data safe. Amazon S3 gives you simple choices so you can pick the best way to protect your files.
