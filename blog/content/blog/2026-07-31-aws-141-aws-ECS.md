...
Amazon Elastic Container Service (ECS) is AWS's native container orchestration platform for running Docker containers. ECS offers two launch types: EC2 (manage your own infrastructure) and Fargate (serverless). The service integrates with [IAM roles](https://magicishaqblog.netlify.app/2023-02-17-aws-9-roles) for security, supports various load balancers for traffic distribution, and can persist data using Amazon EFS. Fargate is the recommended approach for most use cases due to its serverless nature and simplified management.
...
- Pull Docker images from Amazon Elastic Container Registry ([ECR](https://docs.aws.amazon.com/ecr/))
