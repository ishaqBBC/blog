...
Amazon S3 Object Lambda lets you change files as they are being downloaded, without creating extra copies. Using [S3 Access Points](https://magicishaqblog.netlify.app/2026-12-02-13-aws-120-access-points/) and [AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/welcome.html), different applications can see redacted, enriched, or transformed versions of the same file, all from a single [
...
In modern cloud systems, data often serves multiple purposes. An online shop may need full customer records, while an analytics team requires the same data without personal details, and a marketing team might enrich it with loyalty information. Traditionally, this led to duplicating data across multiple buckets, increasing storage costs and complexity. [S3 Object Lambda](https://docs.aws.amazon.com/AmazonS3/latest/userguide/transforming-objects.html) eliminates this need by enabling dynamic data transformation at retrieval, reducing storage overhead and simplifying maintenance. Learn more about [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/) and [AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/welcome.html) in the official AWS documentation.  



## What is Amazon S3 Object Lambda

Amazon Web Services offers a feature called Amazon S3 Object Lambda. It allows developers to modify an [S3 Bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/) just before it is delivered to an application.

The original file stays untouched.

Instead of storing multiple versions of the same object, you transform it in real time as it is being retrieved.
...
below is a diagram
![diagram of object lambdas](/blog/src/images/121/121-1.png)

##  Amazon S3

Amazon S3 is a cloud storage service. Applications can upload files into a bucket and retrieve them later.

Imagine an e-commerce platform storing product data and customer transactions in one  [S3 Bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/). The application can read and write directly to it.

[Read more about Amazon S3 on our previous posts](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/)

But what happens when another team needs a different view of that same data?


## S3 Access Points

Amazon [S3 Access Points](https://magicishaqblog.netlify.app/2026-12-02-13-aws-120-access-points/) provide a controlled access into an [S3 Bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/). Instead of giving every application direct access to the bucket, you create  access points with defined permissions.

## AWS Lambda

[AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/welcome.html) allows you to run small pieces of code in the cloud without managing servers. You upload your function, and AWS runs it whenever it is triggered.

Now imagine connecting a Lambda function to an S3 Access Point.

That is where Object Lambda comes in.
## S3 Object Lambdas

S3 Object Lambda enables applications to access customised views of data stored in a single [S3 Bucket](https://magicishaqblog.netlify.app/2025-04-04-aws-87-s3-bucket-hands-on/) without duplication. For example, an e-commerce application can access the original, directly from the bucket, while an analytics application can retrieve a redacted version of the same data through an Object Lambda Access Point. This is achieved by attaching a Lambda function to the access point, which processes the data in real time to remove sensitive information.

A marketing application can use another Object Lambda Access Point to retrieve an enriched version of the data. A Lambda function adds customer loyalty information to the base object before returning it to the application. This approach allows multiple applications to access tailored views of the same data without creating redundant copies.

By using S3 Object Lambda, organisations , reduce storage costs, and ensure data consistency. The original object remains unchanged, while transformations are applied dynamically during retrieval, enabling efficient and secure data handling.

 

## Real World Use Cases

S3 Object Lambda . It supports several practical needs:

* **Redacting personal data** for analytics or testing environments
* **Data transformation**, such as converting XML into JSON (great for APIs)
* **Image processing**, including resizing or adding watermarks
* **User specific customisation**, where output changes depending on who requests it


Imagine a news website storing high resolution images. When a reader requests one, a Lambda function resizes it and adds a watermark tailored to that user. The stored original never changes.

This flexibility reduces storage overhead and less architecture.

 

## Conclusion

S3 Object Lambda represents a significant shift in how data is managed and accessed in the cloud. By enabling dynamic transformations at retrieval time, it eliminates the need for multiple data copies, reducing storage costs and data management. This approach enhances security, less duplication errors and makes data mailable for needs of a business.

The ability to maintain a single source of truth while delivering customised views of data is attractive for storage costs and resource management. With S3 Object Lambda, organisations can achieve greater security, and adaptability while ensuring the original data remains untouched.
