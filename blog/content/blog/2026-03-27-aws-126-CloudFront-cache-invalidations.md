...
## TLDR
When you update your website usinf AWS, [Amazon CloudFront](https://magicishaqblog.netlify.app/2026-03-06-aws-123-Amazon-CloudFront/) might still show old content. A *cache invalidation* tells it to forget old files so users see the new version right away. [More can be read here](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Invalidation.html).


# Introduction
[Amazon CloudFront](https://magicishaqblog.netlify.app/2026-03-06-aws-123-Amazon-CloudFront/). Has copies of a website in many places around the world, called *[edge locations](https://magicishaqblog.netlify.app/2026-03-20-aws-125-Amazon-Cloudfront-Caching/)*, so people can see your site quickly.

But here’s the problem.

If you change your website , maybe you add a new picture or fix some words  CloudFront doesn’t know straight away. It keeps showing the old version until a timer runs out. This timer is called a *TTL* (time to live).
[AWS Documentation on TTL](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Expiration.html)



# Example

 Website files are stored in [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/).

You update:

* your homepage (`index.html`)
* some pictures in `/images/`

But CloudFront is still showing the old versions because:

* it saved them earlier
* the TTL hasn’t finished yet

That could take hours — even a whole day.

For a news website, shop, or app, its slow.
...
It’s a simple idea:
* CloudFront **forget certain files immediately**

So next time someone visits your site:

* CloudFront checks its cache
* sees the file is gone
* fetches the new version from your origin

And just like that, your users see the update.



# How It Works

1. You update files in [Amazon S3](https://magicishaqblog.netlify.app/2025-03-14-aws-84-Amazon-s3/)
2. You send an invalidation request to CloudFront
3. You choose what to clear:

   * a single file: `/index.html`
   * a group: `/images/*`
   * everything: `*`
4. CloudFront removes those files from all [edge locations](https://magicishaqblog.netlify.app/2026-03-20-aws-125-Amazon-Cloudfront-Caching/)
5. New requests pull fresh content from your origin



You make changes and want everyone to see them right away.
...
Now, when someone visits your site:

* CloudFront no longer has those files saved
* it fetches the new versions
* users see the latest content instantly



# Conclusion

Cache invalidation helps you:

* show updates quickly
* avoid confusion from old content
* keep your website accurate

Without it, your users might see yesterday’s version of your site — even after you’ve fixed it.



# Conclusion

CloudFront caches objects at [edge locations](https://magicishaqblog.netlify.app/2026-03-20-aws-125-Amazon-Cloudfront-Caching/) to reduce latency.  
When origin content changes, cached objects can still be served until their TTL expires.  
An invalidation request removes selected cached objects before TTL expiry, so the next request retrieves the updated object from the origin and caches it again.

Use invalidation for immediate updates to specific paths (for example, `/index.html` or `/images/*`) when waiting for TTL expiry is not acceptable.

More information can be found in these sources:

- [What is CloudFront caching and expiration (TTL)](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Expiration.html)
- [Invalidate files to remove content from edge caches](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/Invalidation.html)
- [How invalidation works](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/HowInvalidationWorks.html)
